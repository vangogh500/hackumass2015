...
exports.checkForExistingProfile = function(username, ign, email, cb){
	...
	Player.find( { $or:[ {'loginUser': username}, {'ign': ign}, {'email': email} ]}, function (err){	
		cb(err); //Did not find any of these, so it is safe to make a new one
...
exports.formatPlayer = function(loginUser, playerName, email, cb) {
    Player.findOne({
		loginUser: loginUser
    }, function(err, found) {
        if (found) cb("Error");
